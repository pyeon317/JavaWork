...
public class MainClass02 {
	...
	public static void main(String[] args) {
		 ...
		 * Function - 매개변슈 여러개 받고, 반환은 임의의 값
		 */
		...
		System.out.println("\n--------------------------------------------------------------------------");
		//filter() - 리턴에 turn값만 남기고 필터링 = 짝수만
		list.stream().distinct().filter( (a) -> a % 2 == 0).sorted().forEach( a-> System.out.print( a+ " ") );//매개변수하나면 -> 앞의 변수를 ()없이 표현가능
		
		System.out.println("\n--------------------------------------------------------------------------");
		//map() - 반환에 정의 된 값으로 완전히 새로운 스트림반환
		list.stream().map( (a) -> a % 2 ==0  ? "짝수" : "홀수").forEach(a -> System.out.print(a + " "));
		
		//mapToDouble() - 반환에 정의된 값으로 doㅕble형 스트림변경
		//mapToInt() = int형 스트링으로 변경
